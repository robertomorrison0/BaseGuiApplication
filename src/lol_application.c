...
static void lol_application_activate(GApplication *app) {
  GtkWindow *window;
...
static void lol_application_startup(GApplication *app) {
  g_assert(LOL_IS_APPLICATION(app));
...
static void lol_application_class_init(LolApplicationClass *klass) {
...
static void lol_application_about_action(GSimpleAction *action,
                                         GVariant *parameter,
                                         gpointer user_data) {
  ...
  adw_show_about_dialog(GTK_WIDGET(window), "application-name",
                        APP_DISPLAY_NAME, "application-icon", APP_ID,
                        "developer-name", DEVELOPER_NAME, "version",
                        PACKAGE_VERSION, "developers", developers, "copyright",
                        g_strdup_printf("© 2023 %s", DEVELOPER_NAME), NULL);
}

static void lol_application_quit_action(GSimpleAction *action,
                                        GVariant *parameter,
                                        gpointer user_data) {
  LolApplication *self = user_data;
...
static void lol_application_init(LolApplication *self) {
  ...
  static const GOptionEntry options[] = {
      ...
      {"quit", 'q', 0, G_OPTION_ARG_NONE, NULL, ("Quit Nautilus."), NULL},
...
gint lol_application_handle_file_args(LolApplication *self,
                                      GVariantDict *options) {
  gint idx, len;
  ...
  if (remaining) {
    for (idx = 0; remaining[idx] != NULL; idx++) {
      ...
      if (cwd == NULL) {
        ...
        self->file =
            g_file_new_for_commandline_arg_and_cwd(remaining[idx], cwd);
        g_free(cwd);
      }
      break;
    }
...
gint lol_application_command_line(GApplication *application,
  ...
  if (g_variant_dict_contains(options, "version")) {
    g_application_command_line_print(
        command_line, APP_NAME " (" APP_DISPLAY_NAME ") " PACKAGE_VERSION "\n");
    return EXIT_SUCCESS;
  }
